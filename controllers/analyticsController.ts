...
class AnalyticsController {
  async getOverview(
    ...
    const { startDate, endDate, country, device } = req.query;
  ...
  async getDashboardAnalytics(
    ...
    const [last30DaysAnalytics, last7DaysAnalytics, allTimeAnalytics] =
      await Promise.all([
    ...
    const clicksGrowth =
      last30DaysAnalytics.totalClicks > 0
        ? Math.round(
            ((last7DaysAnalytics.totalClicks -
              last30DaysAnalytics.totalClicks / 4) /
              (last30DaysAnalytics.totalClicks / 4)) *
              100
          )
        : 0;
